...
function setup(){
    ...
    caixas.push(new caixa(100,100,50,100,3));
    caixas.push(new caixa(150,150,100,50,3));
}
...
function draw(){
    ...
    for (let caixa of caixas) {
        ...
        //bola.edges();
        caixa.show();
        //caixa.generateConnectedPoints();
        //caixa.doPolygonsIntersect(caixas[1]);
    }
    ...
    while(x < caixas.length){
        while(y < caixas.length){
                if(caixas[x].intersects(caixas[y])){
                    console.log("foi");
                }
        }    
    }
